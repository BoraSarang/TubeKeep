...
| T-1213 | **오디오 품질(비트레이트) 선택** — 용량이 커서 "오디오만 추출"에 비트레이트 그룹 선택 추가(기본=중간음질 70k). 포맷 파싱에 abr 추가, AudioBitrate enum(초고160k/고129k/중70k/저50k), formatId를 유튜브 고정 오디오 포맷 id(251/140/250/249) 기반으로 선택, 원본이 webm(opus)이면 `-x --audio-format m4a`로 변환해 항상 .m4a 보장(고=140 m4a 원본 그대로). HomeView 품질 Picker+예상 크기, optionsLabel에 비트레이트 표기. Format.abr 역호환 디코더(decodeIfPresent) | high | ✅ 완료 | AudioBitrate enum(DownloadItem.swift), Format.abr, YouTubeDLService.abr 파싱, HomeReducer.audioBitrate, DownloadManager formatId, HomeView Picker |
