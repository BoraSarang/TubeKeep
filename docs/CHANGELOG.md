...
- **최종 해결 — 닫기=숨김 재사용 설계**: `isReleasedWhenClosed=true` 복원 후에도 deinit 미발생(외부 retain) → 설계 전환. 빨간 닫기 버튼은 delegate `windowShouldClose`, ESC/Cmd+W는 `PlayerWindow.performClose` 오버라이드 경로임을 진단 로그로 식별 — 양쪽 모두 orderOut(숨김)+mpv.stop+pauseRendering으로 전환(playerHideNotification 신설). openPlayerWindow는 숨김 창 포함 항상 재사용
- **검증(사용자 실측)**: 재생→닫기 5회 반복에서 client 생성·창 생성 각 **1회**(첫 재생만), 이후 전부 재사용(창 hash 불변), willClose 0건, 숨김 후 CPU ~0%. 부수 효과: mpv 초기화 생략으로 재생 시작 단축
...
### 랜딩 페이지 리디자인 (T-1122·T-1126, docs/index.html + style.css)
- **구조를 YouTube 홈 미러로 재설계**: 히어로 섹션 제거 → 검색바(40px pill + 분리형 검색 버튼) + 카테고리 칩 행(모두/오프라인 보관함/AI 기능/다운로드/플레이어) + 16:9 썸네일 **비디오 카드 그리드**
- **비디오 카드 (3장)**: 실제 앱 스크린샷(보관함/다운로더/플레이어)을 16:9 썸네일로 크롭, 유튜브 문법의 우하단 배지(오프라인/yt-dlp/H.264) + 36px 원형 채널 아바타(앱 아이콘) + 2줄 제목 + 메타(채널 · 분류 · 버전), hover 시 썸네일 확대 + 진한 오버레이 + 재생 아이콘
- **AI 기능 카드 (3장)**: AI 요약·Q&A / 팟캐스트 / Whisper 자막 — 아이콘 카드, hover 시 2px 상승
- **CTA 배너**: 유튜브 친구 배너 형태의 라운드 카드 — "평생 무료. 당신의 기기 안에만 저장합니다." + 레드 다운로드 버튼
- **마이크로복사**: 검색 placeholder "TubeKeep에서 검색", 제목·메타는 실제 기능/버전(v3.11) 기준 진실한 복사 유지
- **설명 복원 (T-1127, 사용자 재요청)**: 유튜브 홈 미러 개편 후 사라진 제품 소개를 하이브리드 구조로 재구성
...
- hover(오버레이 opacity 1 + 썸네일 scale 1.03), 콘솔 에러 0, 이미지 7장 전부 로드, 16:9 종횡비 유지, 태그 균형 통과
