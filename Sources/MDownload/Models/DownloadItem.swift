...
struct DownloadItem: Identifiable, Equatable, Codable {
    ...
    var estimatedFilename: String {
        ...
        if isChannelDownload {
            return "\(Constants.channelOutputDirectory)/\(folder)/\(String(format: "%03d", channelUploadIndex)) - \(videoInfo.title).\(videoInfo.id).mp4"
        }
        ...
        return "\(Constants.channelOutputDirectory)/\(folder)/\(formatFilename(template: template))"
    }

    func checkExistingFile(outputDirectory: String, template: String) -> String? {
        let folder = Constants.sanitizeFolderName(videoInfo.channel)
        let channelDir = "\(outputDirectory)/\(folder)"
        if isChannelDownload {
