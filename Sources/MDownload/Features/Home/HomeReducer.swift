...
struct HomeReducer {
    ...
    struct State: Equatable {
        ...
        var clipboardMonitoring: Bool = {
            guard let data = UserDefaults.standard.string(forKey: Constants.settingsSaveKey),
                  let json = data.data(using: .utf8),
                  let settings = try? JSONDecoder().decode(Settings.self, from: json)
            else { return true }
            return settings.clipboardMonitoring
        }()
    ...
    var body: some ReducerOf<Self> {
        Reduce { state, action in
            ...
            case .toggleClipboardMonitoring:
                ...
                return .run { [newValue = state.clipboardMonitoring] _ in
                    guard let data = UserDefaults.standard.string(forKey: Constants.settingsSaveKey),
                          let json = data.data(using: .utf8),
                          var settings = try? JSONDecoder().decode(Settings.self, from: json)
                    else { return }
                    settings.clipboardMonitoring = newValue
                    if let encoded = try? JSONEncoder().encode(settings),
                       let jsonStr = String(data: encoded, encoding: .utf8) {
                        UserDefaults.standard.set(jsonStr, forKey: Constants.settingsSaveKey)
                    }
                }
