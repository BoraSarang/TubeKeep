...
struct BatchDownloadView: View {
    ...
    @ViewBuilder
    private func thumbnailView(_ urlString: String?) -> some View {
    ...
    private func runMockBatchTest() {
        ...
        let configs: [MockConfig] = [
            MockConfig(title: "[MV] 아이브(IVE) - 해야 (HEYA)", channel: "starshipTV", duration: 198, height: 1080, subs: false, audioOnly: false, thumbnailURL: "https://i.ytimg.com/vi/abc123/maxresdefault.jpg"),
            MockConfig(title: "뉴진스(NewJeans) 'How Sweet' Official MV", channel: "NewJeans", duration: 192, height: 720, subs: true, audioOnly: false, thumbnailURL: "https://i.ytimg.com/vi/def456/maxresdefault.jpg"),
            MockConfig(title: "aespa 에스파 'Supernova' MV", channel: "aespa", duration: 174, height: 480, subs: false, audioOnly: true, thumbnailURL: "https://i.ytimg.com/vi/ghi789/maxresdefault.jpg"),
            MockConfig(title: "LE SSERAFIM 'EASY' Official MV", channel: "LE SSERAFIM", duration: 165, height: 2160, subs: true, audioOnly: false, thumbnailURL: "https://i.ytimg.com/vi/jkl012/maxresdefault.jpg"),
            MockConfig(title: "BABYMONSTER - 'SHEESH' MV", channel: "BABYMONSTER", duration: 158, height: 0, subs: false, audioOnly: true, thumbnailURL: "https://i.ytimg.com/vi/mno345/maxresdefault.jpg"),
        ]
        ...
        urlItems = mockURLs.map { BatchURLItem(url: $0, status: .pending, videoTitle: nil, thumbnailURL: nil) }
        ...
        for i in configs.indices {
            ...
            let item = DownloadItem(
                videoInfo: VideoInfo(
                    ...
                    duration: c.duration,
                    uploadDate: "20240601",
                    thumbnailURL: configs[i].thumbnailURL,
                    webpageURL: "https://youtube.com/watch?v=test_batch_\(i)",
                    isPlaylist: i == 3,
                    playlistTitle: i == 3 ? "Test Playlist" : nil,
                    playlistCount: i == 3 ? 10 : nil
                ),
                selectedFormat: Format(
                    id: "test_fmt_\(i)",
                    label: c.audioOnly ? "MP3" : "\(c.height)p",
                    height: c.height,
                    ext: c.audioOnly ? "mp3" : "mp4",
                    codec: c.audioOnly ? "mp3" : "h264",
                    filesize: Int64.random(in: 10_000_000...200_000_000),
                    fps: c.audioOnly ? nil : 30,
                    isVideoOnly: false,
                    isAudioOnly: c.audioOnly
                ),
                includeSubtitles: c.subs,
        ...
        Task {
            for i in urlItems.indices {
                ...
                await MainActor.run {
                    urlItems[i].status = .fetching
                }
                try? await Task.sleep(for: .milliseconds(800))
                await MainActor.run {
                    if i < items.count {
                        urlItems[i].status = .added
                        urlItems[i].videoTitle = configs[i].title
                        urlItems[i].thumbnailURL = configs[i].thumbnailURL
                    } else {
                        urlItems[i].status = .skipped("플레이리스트")
                        urlItems[i].videoTitle = "플레이리스트는 제외됨"
                        urlItems[i].thumbnailURL = nil
                    }
                    progressText = "\(i + 1)/\(urlItems.count) 처리 완료"
                }
            }

            try? await Task.sleep(for: .milliseconds(300))
            await MainActor.run {
                // 먼저 첫 번째 item 추가 → addItems에서 videoInfo.id 중복 감지를 테스트
                if let firstItem = items.first {
                    store.send(.downloadQueue(.addItem(firstItem)))
                }
                // items에는 동일 videoInfo.id가 포함되어 있어 중복 제외됨
