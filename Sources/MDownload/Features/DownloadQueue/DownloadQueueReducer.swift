    ...
    var body: some ReducerOf<Self> {
        Reduce { state, action in
            ...
            case let .addItem(item):
                ...
                if let s = readSettingsFromDefaults(), let path = mutableItem.checkExistingFile(
                    outputDirectory: s.outputDirectory, template: s.filenameTemplate
                ) {
            ...
            case let .addItems(newItems):
                let settings = readSettingsFromDefaults()
                var skipCount = 0
                for item in newItems {
                    ...
                    var mutableItem = item
                    if let s = settings, let path = mutableItem.checkExistingFile(
                        outputDirectory: s.outputDirectory, template: s.filenameTemplate
                    ) {
                        mutableItem.status = .completed
                        mutableItem.outputPath = path
            ...
            case let .downloadCompleted(id, success, error):
                if success {
                    ...
                    if let item, item.isChannelDownload {
                    ...
                    if retryCount < state.maxRetries {
                        ...
                        return .merge(nextEffect, .send(.showToast(toast)))
                    }
                }
...
private func readSettingsFromDefaults() -> Settings? {
    UserDefaults.standard.string(forKey: Constants.settingsSaveKey)
        .flatMap { try? JSONDecoder().decode(Settings.self, from: Data($0.utf8)) }
}
