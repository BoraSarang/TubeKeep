...
struct ChannelContentView: View {
            ...
            if let channel = channel {
                ...
                VStack(spacing: 8) {
                    ...
                    Button {
                        ...
                        Label("채널 추가", systemImage: "plus.circle")
                            .font(.system(size: 12))
                    }
    ...
    private var latestVideoDate: String {
        guard let first = videos.sorted(by: { $0.playlistIndex < $1.playlistIndex }).first,
              let uploadDate = first.uploadDate
        else { return "없음" }
        let fmt = DateFormatter()
        fmt.dateFormat = "yyyyMMdd"
        guard let date = fmt.date(from: uploadDate) else { return uploadDate }
    ...
    private var hasNewVideos: Bool {
        ...
        return ChannelDownloadCache.hasNewVideos(channelId: c.id)
    }

    private var newVideoCount: Int {
        guard let c = channel else { return 0 }
        return ChannelDownloadCache.loadNewVideoIds(channelId: c.id).count
    }
        ...
        VStack(spacing: 0) {
            HStack(spacing: 12) {
                ...
                Button("↻") {
                    ...
                    let currentNew = ChannelDownloadCache.loadNewVideoIds(channelId: c.id)
                    if !currentNew.isEmpty {
                        ChannelDownloadCache.saveSeenVideoIds(channelId: c.id, videoIds: currentNew)
                        ChannelDownloadCache.clearNewVideoIds(channelId: c.id)
                    }
            ...
            if hasNewVideos {
                HStack(spacing: 6) {
                    ...
                    Text("새 영상 \(newVideoCount)개 — 새로고침(↻)하여 확인하세요")
                        .font(.system(size: 11))
                    Spacer()
                }
    ...
    private func revealVideoFile(item: ChannelVideoItem, channel: SubscribedChannel) {
        ...
        if let match = files.first(where: { $0.contains(item.id) && $0.hasSuffix(".mp4") }) {
...
private struct ChannelVideoRow: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            CachedThumbnailView(videoId: item.id, url: item.thumbnailURLStandard)
