...
struct HomeView: View {
    ...
    var body: some View {
        ...
        .alert("해상도를 선택해 주세요", isPresented: Binding(
            get: { store.showNoResUnderLimit },
            set: { if !$0 { store.send(.dismissNoResUnderLimit) } }
        )) {
            Button("확인", role: .cancel) { store.send(.dismissNoResUnderLimit) }
        } message: {
            Text("설정한 기본 해상도 이하의 포맷이 없습니다.\n목록에서 원하는 해상도를 선택한 후 다운로드해 주세요.")
        }
    }
