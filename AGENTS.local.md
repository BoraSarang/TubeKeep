...
```bash
# 디버그 빌드 및 실행
bash build_and_run.sh debug

# 릴리즈 빌드
bash build_and_run.sh release
```

- 빌드 후 `TubeKeep.app` 생성 확인
...
| `DebugLogManager.swift` / `DebugLogView.swift` | 공유 디버그 로그 시스템 (singleton, DEBUG only) |
