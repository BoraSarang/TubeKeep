...
┌─────────────────────────────────────────┐
│              AppDelegate                │
│  NSStatusBar / NSWindow x4 / NSPanel  │
│  글로벌단축키 / 클립보드감시 / 속도측정  │
└────────────────┬────────────────────────┘
...
#### Action
```
loadFromDisk / itemsLoaded / addItem / removeItem
...
|----|-----------|----------|----------|---------|------|
| 라이브러리 | `"main"` | 840×640 | 840×500 | 가로 고정 (FixedWidthWindowController) | 가능 |
...
- **Library 데이터**: UserDefaults `"downloadLibrary"` 에 `[videoId: LibraryItem]` JSON
  - `loadItems()` → [LibraryItem]
  - `addItem(LibraryItem)` / `removeItem(id:)`
- **썸네일 캐시**: NSCache(메모리) + `~/Library/Caches/com.mdownload.library/thumbnails/` (디스크)
...
- `fetchChannelInfo(url:)` → (SubscribedChannel, [ChannelVideoItem])
...
- **그리드 모드**: LazyVGrid adaptive(min:180, max:300), spacing 16, padding 16
  ...
  - 좌클릭 → LeftClickMenu (좌표 기반 NSMenu): 열기 / Finder에서 보기 / 삭제
...
| 파일 | 설명 |
|------|------|
| `MDownloadApp.swift` | `@main` entry point, NSApplicationDelegateAdaptor |
| `AppDelegate.swift` | 메뉴바, 4개 윈도우, 단축키, 클립보드, 속도측정 |
| `AppReducer.swift` | Root TCA reducer, 브릿지 로직 |
| `MainView.swift` | 영상 다운로더 메인 뷰 (Home+Queue+Settings) |
...
| `Library/LibraryView.swift` | 라이브러리 root (sidebar + content + toolbar) |
...
| 키 | 타입 | 내용 |
|----|------|------|
| `"appSettings"` | JSON | Settings (concurrentDownloads, outputDirectory, ...) |
| `"downloadLibrary"` | JSON | `[videoId: LibraryItem]` |
| `"libraryViewMode"` | String | "grid" / "list" |
| `"subscribedChannels"` | JSON | `[SubscribedChannel]` |
